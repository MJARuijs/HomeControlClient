...
class ColorPickerPalette : TableLayout {
    ...
    private var mSwatchLength: Int = 0
    private var mMarginSize: Int = 0
     ...
     */
    fun init(size: Int, columns: Int, listener: ColorPickerSwatch.OnColorSelectedListener) {
        mNumColumns = columns
        val res = resources
        if (size == 2) {
    ...
    fun drawPalette(colors: IntArray?, selectedColor: Int, colorContentDescriptions: Array<String>? = null) {
        ...
        for ((tableElements, color) in colors.withIndex()) {
    ...
    private fun setSwatchDescription(rowNumber: Int, index: Int, rowElements: Int, selected: Boolean,
                                     swatch: View, contentDescriptions: Array<String>?) {
        val description: String
        if (contentDescriptions != null && contentDescriptions.size > index) {
    ...
    private fun createColorSwatch(color: Int, selectedColor: Int): ColorPickerSwatch {
        val view = ColorPickerSwatch(context, color, color == selectedColor, mOnColorSelectedListener)
        val params = TableRow.LayoutParams(mSwatchLength, mSwatchLength)
        params.setMargins(mMarginSize, mMarginSize, mMarginSize, mMarginSize)
        view.layoutParams = params
        return view
